...
public class SampleGoalImage extends Command {
	...
	protected void initialize() {
		/*
		 * System.out.println("$$$"); //Initializing the two frames and session
		 * for the usb cam Robot.cam.frame =
		 ...
		 * 
		 * NIVision.IMAQdxSetAttributeString(Robot.cam.session,
		 * "CameraAttributes::Brightness::Mode", "Manual"); long minv =
		 * NIVision.IMAQdxGetAttributeMinimumI64(Robot.cam.session,
		 * "CameraAttributes::Brightness::Value");//30 long maxv =
		 * NIVision.IMAQdxGetAttributeMaximumI64(Robot.cam.session,
		 * "CameraAttributes::Brightness::Value");//255 long val = minv + (long)
		 * (((double) (maxv - minv)) * (((double) Robot.cam.defaultBrightVal) /
		 * 100.0)); NIVision.IMAQdxSetAttributeI64(Robot.cam.session,
		 * "CameraAttributes::Brightness::Value", val);
		 * 
		 * //Declaring the start of the camera System.out.println(
		 * "starting camera");
		 * 
		 * 
		...
		// adding the arrays of the standard camera displacement to the point
		...
		// Uses the point displacements to calculate the points representing
		// them on the screen
	...
	protected void execute() {
		...
		NIVision.imaqDrawLineOnImage(Robot.cam.frame, Robot.cam.frame, DrawMode.DRAW_VALUE, lowerLeft, lowerRight,
				255.0f);
		NIVision.imaqDrawLineOnImage(Robot.cam.frame, Robot.cam.frame, DrawMode.DRAW_VALUE, lowerLeft, upperLeft,
				255.0f);
		NIVision.imaqDrawLineOnImage(Robot.cam.frame, Robot.cam.frame, DrawMode.DRAW_VALUE, upperLeft, upperRight,
				255.0f);
		NIVision.imaqDrawLineOnImage(Robot.cam.frame, Robot.cam.frame, DrawMode.DRAW_VALUE, lowerRight, upperRight,
				255.0f);

		NIVision.imaqDrawLineOnImage(Robot.cam.frame, Robot.cam.frame, DrawMode.DRAW_VALUE, testPointStart,
				testPointFinish, 255.0f);
