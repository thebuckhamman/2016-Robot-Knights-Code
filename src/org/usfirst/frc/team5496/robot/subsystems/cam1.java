...
public class cam1 extends Subsystem {
	...
	public NIVision.Range TOTE_HUE_RANGE = new NIVision.Range(110, 130);	//Green sat for dark image
	public NIVision.Range TOTE_SAT_RANGE = new NIVision.Range(88, 255);	//Default saturation range for yellow tote
    ...
    )
    {
    	 ...
    	 *
    	 * double distHorzAngle = Math.asin(b/camDist);
    	 * double distVertAngle = Math.asin(c/camDist) - CAMERA_TILT_ANGLE_RADIANS; 
    	 * 
    	 ...
    	 * a = c*SQRT((cosec^2(distHorzAngle) - 1 - angleRatio^2)), c = a/SQRT((cosec^2(distHorzAngle) - 1 - angleRatio^2)) 
    	 * 
    	 ...
    	 * a = b*SQRT((cosec^2(totalVertAngle) - 1 - angleRatio^-2)), b = a/SQRT((cosec^2(totalVertAngle) - 1 - angleRatio^-2))
    	 * 
    	 * For a=1:
    	 * c = 1/SQRT((cosec^2(distHorzAngle) - 1 - angleRatio^2))  
    	 * b = 1/SQRT((cosec^2(totalVertAngle) - 1 - angleRatio^-2))
    	 * 
    	 * For c=1:
    	...
    	{
    		ratios[0] = 1;
    		ratios[2] = 1/Math.sqrt(1/(Math.sin(distHorzAngle)*Math.sin(distHorzAngle)) - 1 - angleRatio*angleRatio);
    		ratios[1] = 1/Math.sqrt(1/(Math.sin(totalVertAngle)*Math.sin(totalVertAngle)) - 1 - 1/(angleRatio*angleRatio));
    	}
    	//ratio[1], or b, = 1
    	else if(constIndex == 2)
    	{
    		ratios[0] = Math.sqrt(1/(Math.sin(distHorzAngle)*Math.sin(distHorzAngle)) - 1 - angleRatio*angleRatio);
    		ratios[2] = 1;
    		ratios[1] = angleRatio;
    	}
    	//ratio[2], or c, = 1
    	...
    	{
    		ratios[0] = Math.sqrt(1/(Math.sin(totalVertAngle)*Math.sin(totalVertAngle)) - 1 - 1/(angleRatio*angleRatio));
